---
name: instant-self
description: FIDScript self-hosted InstantDB development mode. Use when working with FIDScript self-hosted backend, instant.fidscript.com dashboard, or when building apps that use the @fidscript/instant-mcp server.
---

# FIDScript Self-Hosted Development Mode
...
**API:** `https://apiinstant.fidscript.com`
**MCP Server:** `instant-self` (stdio transport, authenticated via PAT)
...
RULE 1: NEVER delete an application during initialization.
        "New project" means create a NEW app. Do NOT delete existing apps.
...
RULE 11: Never create duplicate applications because project identity was not found.
         If project identity is ambiguous, stop and ask.

RULE 12: Never delete or modify unrelated FIDScript applications.
         If list-apps returns apps that are not this project, leave them alone.

RULE 13: If project identity is ambiguous, stop and ask rather than guessing.
```
...
Need to query data?
  → query (app-id + InstaQL query required)

Need to write data?
  → transact (app-id + Instaml steps required)
...
Need to manage webhooks?
  ...
  → update-webhook (app-id required)
  → delete-webhook (app-id required)
  → enable-webhook / disable-webhook (app-id required)

Need webhook history?
  → get-webhook-events (app-id + webhook-id required)
  → resend-webhook-event (retry failed delivery)

Need email configuration?
  → get-email-template
  → update-email-template (app-id required)
  → send-test-email (app-id required)

Need backups?
  → list-backups (app-id required)
  → create-backup (app-id required)
  → get-backup-job (app-id + job-id required)
  → delete-backup (app-id + backup-id required)

Need test users?
  → list-test-users (app-id required)
  → create-test-user (app-id required)
  → delete-test-user (app-id required)

Need organizations?
  → list-orgs
  → get-org (org-id required)
  → list-org-apps (org-id required)
  → invite-app-member (app-id required)
  → update-app-member (app-id required)
  → remove-app-member (app-id required)
...
FIDScript self-hosted maintains compatibility with the InstantDB programming model, but **server/API-level behavior can differ from hosted Cloud InstantDB**. This section documents the verified FIDScript self-hosted behavior. When targeting FIDScript, use the syntax documented here — do not assume Cloud InstantDB documentation examples apply directly to server/API operations.
...
The `instant-self` MCP is already operational. This skill is the operating procedure for using it correctly.
